...
def product(*iterables, repeat=1):
    ...
    sequences = [tuple(it) for it in iterables]
    length = 1
    for seq in sequences:
        length *= len(seq)
    length = length ** repeat

    class ProductWithLen:
        def __init__(self):
            self._it = _product(*sequences, repeat=repeat)
    ...
    return ProductWithLen()

def import_file(filename : str ) -> object:
    """Import a module from an arbitrary path and return the loaded module object.

    Parameters
    ----------
    filename : str | pathlib.Path
        Absolute or relative path to a Python file containing module-level code.
    ...
    try:
        ...
        print("Could not import module from file '%s'. " % filename)
        exit()
    spec.loader.exec_module(module)
    return module


def load_potential(file: str, pot_name: str) -> Any:
    ...
    try:
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        return getattr(module, pot_name)
    except AttributeError:
        print("Could not load potential %s from file '%s'. " % (pot_name, file))
        print("Make sure you have specified the relative path and " +
              "named the potential correctly!")
        exit()
...
def derivative(f, x, dx):
    """
    Calculate the derivative of `f` at `x` using a centered finite difference
    with step size `dx`. The derivative is calculated to 4th order in `dx`.

    Parameters
    ----------
    f : callable
        The function to differentiate. The first argument should be `x`.
    x : array_like
        The point at which to evaluate the derivative.
    dx : float
        The step size to use in the finite difference.
    ...
    """
    df = -f(x + 2*dx) + 8*f(x + dx) - 8*f(x - dx) + f(x - 2*dx)
    df = df/(12.0*dx)
    return df
...
def eigenvalues_2x2(a, b, c):
    ...
    eig1 = 0.5 * (trace - root)
    eig2 = 0.5 * (trace + root)
    small = np.minimum(eig1, eig2)
    large = np.maximum(eig1, eig2)
    return small, large
...
class gradientFunction(object):
    ...
    def __init__(self, f, eps, Ndim, order=4):
        """Store the finite-difference stencil required to evaluate gradients."""
...
class hessianFunction(object):
    ...
    order : 2 or 4
        Calculate the derivatives to either 2nd or 4th order in `eps`.
    """
    def __init__(self, f : callable, eps : float, Ndim : int, order : int=4):
        """Pre-compute second-order finite-difference coefficients and offsets.
        
        Parameters
        ----------
        f : callable
            The function for which to compute the Hessian.
        eps : float
            The small change in `x` used to calculate the finite differences.
        Ndim : int
            Number of dimensions for each point.
    ...
    def __call__(self, x, *args, **kwargs):
        ...
        -------
        array_like
            The Hessian matrix evaluated at each input point.
        """
        Ndim = self.Ndim
        f = self.f
        coef = self.coef
...
class cubicInterpFunction(object):
    ...
    def __init__(self, y0, dy0, y1, dy1):
        """Store the cubic Bézier control points defined by the boundary data."""
    ...
    def __call__(self, t):
        """Evaluate the cubic interpolant at the normalised coordinate ``t``."""
        mt = 1-t
        y0, y1, y2, y3 = self.Y
